({
    ...
    updatePricebookEntries: function (component, pricebookId, lines, discount, type) {
        if (lines.length == 0) {
            ...
            if (discount < 0) {
                ...
                toastEvent.setParams({
                    'title': 'Error!',
                    'message': 'Discount value can\'t be less than 0.',
                    'type': 'error'
                });
                toastEvent.fire();
            } else if (discount > 99 && type == "percent") {
                var toastEvent = $A.get("e.force:showToast");
                toastEvent.setParams({
                    'title': 'Error!',
                    'message': 'The maximum discount percent is 99%',
                    'type': 'error'
                });
                toastEvent.fire();
            } else {
